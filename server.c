...
void *server_main(void *PORT_arg)
{
	...
	recv(clientfd[0], &user0, sizeof(user0), 0);
	printf("Connected %s\n", user0);
	...
	listen(server_fd, 3);
	send(clientfd[0], &user0, sizeof(user0), 0);
	send(clientfd[0], &user1, sizeof(user1), 0);
	send(clientfd[0], &server_data.turn, 4, 0);
	server_data.turn++;
	send(clientfd[1], &user0, sizeof(user0), 0);
	send(clientfd[1], &user1, sizeof(user1), 0);
	send(clientfd[1], &server_data.turn, 4, 0);
...
{ // communicating server_data with the client (mostly sending)
	...
	while (game_running)
	{
		...
		send(clientfd[client_id], &server_data.is_game_over, sizeof(server_data.is_game_over), 0);
		send(clientfd[client_id], &server_data.turn, sizeof(server_data.turn), 0);
		send(clientfd[client_id], &server_data.winsP0, sizeof(server_data.winsP0), 0);
		send(clientfd[client_id], &server_data.winsP1, sizeof(server_data.winsP1), 0);
		send(clientfd[client_id], &server_data.winner, sizeof(server_data.winner), 0);
		for (int i = 0; i < 9; i++)
			send(clientfd[client_id], &server_data.game_grid[i], sizeof(server_data.game_grid[i]), 0);

		// read client events
		recv(clientfd[client_id], &server_data.click_position, sizeof(server_data.click_position), 0);
