CC = g++
SRC_DIR = src
SRCS = main.cpp gui.cpp server.cpp client.cpp bot.cpp shapes.cpp gameplay.cpp
BUILD_TYPE ?= DEBUG
...
ifeq ($(PLATFORM), Linux)
	...
	LDFLAGS = -lpthread -lm -ldl -L$(RAYLIB_PATH) -l:$(LIBRAYLIB) -Llib/imgui -limgui
	...
	CC = $(EMSDK_PATH)/emcc
	CFLAGS = -Os -s -O1 -s ASYNCIFY -s USE_GLFW=3 -s TOTAL_MEMORY=67108864 -s FORCE_FILESYSTEM=1 # -lwebsocket.js -s PROXY_POSIX_SOCKETS=1 -s USE_PTHREADS=1 -s PROXY_TO_PTHREAD=1
	...
	CFLAGS = -I$(RAYLIB_PATH) -I$(ANDROID_NDK_PATH)/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/include/android -I$(ANDROID_NDK_PATH)/sources/android/native_app_glue -march=armv7-a -mfloat-abi=softfp -mfpu=vfpv3-d16 -ffunction-sections -funwind-tables -fstack-protector-strong -fPIC -Wall -Wa,--noexecstack -Wformat -Werror=format-security -no-canonical-prefixes -DANDROID=$(SDK_VER) --sysroot=$(ANDROID_TOOLCHAIN)/sysroot
endif
LDFLAGS += 
TARGET = Simple_TTT
ifeq ($(PLATFORM), web)
...
all: $(OBJS)
	$(CC:++=) -c $(ANDROID_NDK_PATH)/sources/android/native_app_glue/android_native_app_glue.c -o $(OBJ_DIR)/native_app_glue.o -march=armv7-a -mfloat-abi=softfp -mfpu=vfpv3-d16 -ffunction-sections -funwind-tables -fstack-protector-strong -fPIC -Wall -Wa,--noexecstack -Wformat -Werror=format-security -no-canonical-prefixes -DANDROID=$(SDK_VER) -DPLATFORM_ANDROID -DANDROID=$(SDK_VER)
	$(ANDROID_TOOLCHAIN)/bin/llvm-ar rcs $(OBJ_DIR)/libnative_app_glue.a $(OBJ_DIR)/native_app_glue.o
	$(CC) -o $(LIB_DIR)/lib$(TARGET).so $(OBJ_DIR)/*.o lib/imgui/backends/imgui_impl_android.cpp -shared -I. -Ilib/imgui -I../raylib/release/include -I$(ANDROID_NDK_PATH)/sources/android/native_app_glue -Wl,-soname,lib$(TARGET).so -Wl,--exclude-libs,libatomic.a -Wl,--build-id -Wl,--no-undefined -Wl,-z,noexecstack -Wl,-z,relro -Wl,-z,now -Wl,--warn-shared-textrel -Wl,--fatal-warnings -u ANativeActivity_onCreate -L. -L$(OBJ_DIR) -L$(LIB_DIR) -lraylib -lc++_shared -limgui -lnative_app_glue -llog -landroid -lEGL -lGLESv2 -lOpenSLES -latomic -lc -lm -ldl
...
$(OBJS): dirs
	$(CC) $(CFLAGS) -c -o $(OBJ_DIR)/$@ $(SRC_DIR)/$(@F:.o=.cpp)
