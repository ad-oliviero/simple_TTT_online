...
bool android_button(Rectangle bounds, const char *text) {
	...
	DrawText(text, bounds.x + (bounds.width / 2) - MeasureText(text, 20) / 2, bounds.y + (bounds.height / 2) - MeasureText(text, 20) / 2, 20, BLACK);
...
int join_window(char *IP_ADDRESS, int *PORT, struct client_data *data) {
	...
	while (!game_running && !WindowShouldClose()) {
		...
		{
			DrawText("Select Game Mode", (320 - MeasureText("Select Game Mode", 20)) / 2, 5, 20, DARKGRAY);
			if (android_button((Rectangle){5, 30, 150, 40}, "Single Player")) {
				selection_step++;
				game_mode = 1;
			} else if (android_button((Rectangle){165, 30, 150, 40}, "Multi Player")) {
				selection_step++;
				game_mode = 2;
			}
		} else if (selection_step == 1 && game_mode == 1) // single player
		...
		{
			DrawText("Select Game Hosting", (320 - MeasureText("Select Game Hosting", 20)) / 2, 5, 20, DARKGRAY);
			if (android_button((Rectangle){5, 30, 150, 40}, "Host")) {
				selection_step++;
				game_hosting = 1;
			} else if (android_button((Rectangle){165, 30, 150, 40}, "Join")) {
				selection_step += 2;
				game_hosting = 2;
			}
		} else if (selection_step == 2 && game_hosting == 1) // hosting multi player
